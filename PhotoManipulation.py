...
class PhotoManipulation:
  ...
  def __init__(self, completePuzzlePiecesImage): 
    self.completePuzzlePiecesImage = completePuzzlePiecesImage
    
  def createImages(self):
    puzzle = Image.open(self.completePuzzlePiecesImage)
   ...
   # Do Not Delete
    ...
    for x in range(0, len(colBoundaries), 2):
      ...
      for y in range(0, len(rowBoundaries), 2):
        ...
        toAdd.save("3x4Pieces/%d-%d.png" % (x/2 + 1, y/2 + 1))
        pieces.append(PuzzlePiece("3x4Pieces/%d-%d.png" % (x/2 + 1, y/2 + 1)))

photoManipulator = PhotoManipulation("resources/transparent.png")
photoManipulator.createImages()